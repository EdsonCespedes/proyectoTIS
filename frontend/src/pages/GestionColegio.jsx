...
const GestionColegios = () => {
  ...
  const handleRetirar = async (colegioId) => {
    setCargando(true);

    try {
      const response = await fetch(`${apiUrl}/colegio/${colegioId}/deshabilitar`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        ...
        alert("Error al deshabilitar el colegio");
        setCargando(false);
        return;
      }
      ...
      alert("Ocurrió un error al deshabilitar el colegio.");
      setCargando(false);
    } finally {
      setCargando(false);
    }
          ...
          <tbody>
            {currentColegios.map((colegio) => (
              <tr key={colegio.idColegio}>
                ...
                <td>{colegio.estado ? "Activo" : "Inactivo"}</td>
                <td>
                  <div className="btn-groupdetconv">
                    <button className="edit-btndetconv" onClick={() => handleModificar(colegio)}>✏️</button>
                    <button className="delete-btndetconv" onClick={() => handleRetirar(colegio.idColegio)}>❌</button>
                  </div>
                </td>
              </tr>
            ))}
            ...
            {expandedId === colegio.idColegio && (
              <div className="user-details">
                ...
                <div className="card-actions">
                  ...
                  <button onClick={() => handleRetirar(colegio.idColegio)}>❌ Retirar</button>
                </div>
              </div>
            )}
          </div>
